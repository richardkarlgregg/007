...
interface StageOutput {
  ...
  boundPads: ReturnType<typeof parseSetupPad3dlist>;
  ...
  /** Decoded geometry for prop models found in `assets/obseg/prop/P<name>Z.bin`. */
...
function parseRunwayLevelScale(bgSourcePath: string): number {
  const src = existsSync(bgSourcePath) ? readFileSync(bgSourcePath, "utf8") : "";
  for (const line of src.split(/\r?\n/)) {
    if (!line.includes("LEVELID_RUNWAY")) continue;
...
const propModelScales: Record<string, number> = {};
for (const e of propModelEntries) propModelScales[e.name] = e.scale;
...
for (const p of propPlacements) {
  if (PROP_TYPES_WITH_MODEL.has(p.type)) {
    ...
    if (entry) usedModelNames.add(entry.name);
  }
}

// Try to parse each model binary. Gracefully skips missing / undecompressable files.
const propModels: Record<string, PropModelGeometry> = {};
const propMaterialIds = new Set<number>();
...
for (const name of usedModelNames) {
  ...
  if (geo) {
    propModels[name] = geo;
    for (const id of geo.materialIds) propMaterialIds.add(id);
...
const annotatedPlacements = propPlacements.map((p) => {
  if (!PROP_TYPES_WITH_MODEL.has(p.type)) return p;
  const entry = propModelEntries[p.primaryIndex];
  if (!entry) return p;
