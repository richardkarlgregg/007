...
export interface ModelGraphNode {
  ...
  dlDebug?: Array<{
    nodeOpcode: number;
    primaryPtr: number | null;
    secondaryPtr: number | null;
    verticesPtr: number | null;
    basePtr: number | null;
    numVertices: number | null;
    primaryTriCount: number;
    secondaryTriCount: number;
    primaryAbsMax: number;
    secondaryAbsMax: number;
    altLayoutBTriCount?: number;
    altLayoutBAbsMax?: number;
    chosenLayout?: "A" | "B";
  }>;
}
...
function decodeGfx(
  ...
  const segTable: Record<number, number | null> = {
    4: vtxBinaryOffset,
  ...
  while (iters < 65536) {
    ...
    if (opcode === 0x04) {
      ...
      const vtxSource: number | null = segTable[seg] ?? vtxBinaryOffset;
      for (let i = 0; i < n; i++) {
        if (vtxSource === null) {
          cache[v0 + i] = null;
          continue;
...
function ptrToOffsetFlexible(ptr: number, binaryLen: number): number | null {
  const seg = ptrToOffset(ptr, binaryLen);
  if (seg !== null) return seg;
  // Some model records carry already-relocated/raw file offsets.
  if (ptr >= 0 && ptr < binaryLen) return ptr;
  return null;
}

function isLikelyDisplayList(binary: Uint8Array, off: number): boolean {
  if (off < 0 || off + 8 > binary.length) return false;
  // N64 Gfx words are 8-byte aligned in these assets.
  if ((off & 0x7) !== 0) return false;
  ...
  while (pos <= limit && steps < 512) {
    ...
    if (
      op === 0xde || op === 0x04 || op === 0x01 || op === 0xc0 ||
...
function ptrToOffsetForGfx(binary: Uint8Array, ptr: number): number | null {
  const seg = ptrToOffset(ptr, binary.length);
  if (seg !== null) return seg;
  if (ptr >= 0 && ptr < binary.length && isLikelyDisplayList(binary, ptr)) return ptr;
  return null;
}
...
function computeTriangleAbsMax(triangles: PropTriangle[]): number {
  let r = 0;
  for (const tri of triangles) {
    for (const v of [tri.a, tri.b, tri.c]) {
      ...
      if (opcode === 0x04 && dataOff + 0x14 <= binary.length) {
        ...
        if (priOff !== null) {
          ...
          if (isGunModel) {
            ...
            let useB = false;
            if (aLooksLikeVertices && !bLooksLikeVertices) useB = false;
            else if (!aLooksLikeVertices && bLooksLikeVertices) useB = true;
            else if (!aLooksLikeVertices && !bLooksLikeVertices) useB = triB > triA;
            const chosenPri = useB ? priB : priA;
            const chosenSec = useB ? secB : secA;
            pushChunk(chosenPri);
            if (secOff !== null) pushChunk(chosenSec);
            nodeObj.dlDebug ??= [];
        ...
        if (priOff !== null && vtxOff !== null) {
          ...
          pushChunk(priTris);
          if (secOff !== null) pushChunk(secTris);
          nodeObj.dlDebug ??= [];
          nodeObj.dlDebug.push({
            nodeOpcode: opcode,
            primaryPtr: priPtr >>> 0,
