 ...
 */
function collectDisplayListRecordsFromModelTree(binary: Uint8Array): DLCollisionInfo[] {
  ...
  const stack: number[] = [rootOff];

  while (stack.length > 0) {
    const nodeOff = stack.pop()!;
    if (seenNodes.has(nodeOff)) continue;
    ...
    if (nextOff !== null) stack.push(nextOff);
    if (childOff !== null) stack.push(childOff);
...
function parseModelBoundingBox(binary: Uint8Array): PropModelGeometry["sourceBounds"] | null {
  if (binary.length < 4) return null;
  const rootPtr = readU32BE(binary, 0);
  if (!isSegAddr(rootPtr)) return null;
  const rootOff = rootPtr - SEGMENT_BASE;
  if (rootOff < 0 || rootOff + 0x18 > binary.length) return null;

  const childPtr = readU32BE(binary, rootOff + 0x14);
  if (!isSegAddr(childPtr)) return null;
  const childOff = childPtr - SEGMENT_BASE;
  if (childOff < 0 || childOff + 0x18 > binary.length) return null;

  const opcode = readU16BE(binary, childOff);
  // ModelRoData_BoundingBoxRecord = opcode 10 (0x0A)
  if (opcode !== 0x000a) return null;

  const dataPtr = readU32BE(binary, childOff + 0x04);
  if (!isSegAddr(dataPtr)) return null;
  const dataOff = dataPtr - SEGMENT_BASE;
  if (dataOff < 0 || dataOff + 0x1c > binary.length) return null;
  ...
  if (![xmin, xmax, ymin, ymax, zmin, zmax].every((v) => Number.isFinite(v))) return null;
  return {
...
function scanDLCollisionRecords(binary: Uint8Array): DLCollisionInfo[] {
  ...
  for (let off = 0; off + 32 <= binary.length; off += 4) {
    ...
    if (numVtx < 1 || numVtx > 128) continue;
    if (numColVtx < 0 || numColVtx > 128) continue;
...
function scanDisplayListRecords(binary: Uint8Array): DLCollisionInfo[] {
  ...
  for (let off = 0; off + 20 <= binary.length; off += 4) {
    ...
    if (numVtx < 1 || numVtx > 255) continue;
...
export function parsePropModel(binPath: string): PropModelGeometry | null {
  ...
  const dedup = new Map<string, DLCollisionInfo>();
  for (const rec of [
  ...
  const allTriangles: PropTriangle[] = [];
