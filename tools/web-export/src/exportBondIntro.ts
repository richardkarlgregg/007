...
function buildGunNodeManifest(graph: ModelGraphData): Array<Record<string, unknown>> {
  const triCountByNode = new Map<number, number>();
...
function selectStartingWeaponId(intro: SetupIntroData): number {
  return intro.startWeapons.find((w) => w.isDemoPlayback === 0)?.itemRight ?? 5;
}

function selectWeaponModels(itemRight: number): { fpBinName: string; introPropName: string } {
  ...
  if (itemRight === 5) return { fpBinName: "GwppkZ.bin", introPropName: "chrwppk" };
  return { fpBinName: "GwppkZ.bin", introPropName: "chrwppk" };
}

function extractWeaponPoseFromStats(
  weaponStatsPath: string,
  statsName: string
): { posX: number; posY: number; posZ: number; sway: number } {
  ...
  if (!block) return { posX: 11.0, posY: -20.8, posZ: -33.5, sway: 8.5 };
  ...
  const parseToken = (s: string | undefined): number => {
    if (!s) return 0;
    if (/^-/i.test(s) && /^-0x/i.test(s)) return -Number.parseInt(s.slice(3), 16);
...
function computeBoundsFromTriangles(triangles: PropModelGeometry["triangles"]): PropModelGeometry["bounds"] {
...
function filterGunOutlierTriangles(geo: PropModelGeometry): PropModelGeometry {
  const mags: number[] = [];
  ...
  if (mags.length === 0) return geo;
  mags.sort((a, b) => a - b);
  const median = mags[Math.floor(mags.length / 2)];
  if (!(median > 0)) return geo;
  ...
  const edgeLength = (a: { x: number; y: number; z: number }, b: { x: number; y: number; z: number }): number =>
    Math.hypot(a.x - b.x, a.y - b.y, a.z - b.z);
  const triArea = (a: { x: number; y: number; z: number }, b: { x: number; y: number; z: number }, c: { x: number; y: number; z: number }): number => {
  ...
  const maxEdges = triangles.map((t) => Math.max(
    edgeLength(t.a, t.b),
    edgeLength(t.b, t.c),
    edgeLength(t.c, t.a)
  )).sort((a, b) => a - b);
  ...
  const trianglesGeom = triangles.filter((t) => {
    ...
    if (maxE > maxEdgeThreshold) return false;
    if (maxE / minE > 40.0) return false;
    if (triArea(t.a, t.b, t.c) <= 1e-4) return false;
    return true;
  });
  ...
  for (const tri of finalTriangles) if (tri.materialId > 0) materialSet.add(tri.materialId);
  return {
...
function filterGunByHeaderRadius(geo: PropModelGeometry, headerRadius: number, radiusMultiple: number): PropModelGeometry {
  if (!(headerRadius > 0) || !(radiusMultiple > 0)) return geo;
  ...
  const triangles = geo.triangles.filter((tri) => {
    const within = (v: { x: number; y: number; z: number }): boolean =>
      Math.hypot(v.x, v.y, v.z) <= limit;
    if (!(within(tri.a) && within(tri.b) && within(tri.c))) return false;
  ...
  const materialSet = new Set<number>();
  for (const tri of triangles) if (tri.materialId > 0) materialSet.add(tri.materialId);
  return {
    triangles,
    materialIds: [...materialSet].sort((a, b) => a - b),
...
function filterGunGraphOutlierChunks(
  ...
  const withinHeader = (v: { x: number; y: number; z: number }, limit: number): boolean =>
    Math.hypot(v.x, v.y, v.z) <= limit;
    ...
    .map((chunk) => {
      ...
      if (headerLimit > 0) {
        triangles = triangles.filter((t) => {
          ...
          const maxE = Math.max(e1, e2, e3);
          const minE = Math.max(1e-6, Math.min(e1, e2, e3));
          if (maxE > headerMaxEdge) return false;
          if (maxE / minE > 40.0) return false;
          if (triArea(t.a, t.b, t.c) <= 1e-4) return false;
          return true;
        });
...
function extractGunHeaderRadius(headersPath: string, modelName: string): number {
  const src = readFileSync(headersPath, "utf8");
  const rx = new RegExp(
    `MODELFILEHEADER\\(${modelName}\\s*,\\s*0\\s*,\\s*[^,]+,\\s*0\\s*,\\s*[^,]+,\\s*[^,]+,\\s*([\\d.]+)\\s*,`,
    "i"
...
function bondModelForCuff(cuffId: number): { body: string; head: string } {
  ...
  if (cuffId === 1) return { body: "suitbond", head: "headbrosnansuit" };     // CUFF_BROSNAN
  if (cuffId === 2) return { body: "timberbond", head: "headbrosnantimber" }; // CUFF_JUNGLE
  if (cuffId === 3) return { body: "boilerbond", head: "headbrosnanboiler" }; // CUFF_BOILER
  if (cuffId === 4) return { body: "snowbond", head: "headbrosnansnow" };     // CUFF_SNOW
...
if (!gunIntroGeoRaw) throw new Error(`Failed to decode intro gun model P${weaponModels.introPropName}Z.bin`);
if (!gunFpGeoRaw) throw new Error(`Failed to decode first-person gun model ${weaponModels.fpBinName}`);
...
if (!gunFpGraphRaw) throw new Error(`Failed to decode first-person gun graph ${weaponModels.fpBinName}`);
...
if (!gunIntroEntry) throw new Error(`Missing ${weaponModels.introPropName} scale entry in propItemModelFileRecord.inc.c`);
...
let animationData: BondIntroAsset["animation"] | undefined;
try {
  ...
  if (animEntry) {
    ...
    if (skelJoints && skelJoints.length > 0) {
      ...
      console.log(
        `Animation: ${animEntry.animName} — ${header.frameCount} frames, ` +
...
const addFromGraph = (graph: ModelGraphData): void => {
  for (const chunk of graph.chunks) {
    for (const id of chunk.materialIds) if (id > 0) allMaterialIds.add(id);
  }
};
const addFromGeo = (geo: PropModelGeometry | null): void => {
  if (!geo) return;
  for (const id of geo.materialIds) if (id > 0) allMaterialIds.add(id);
};
