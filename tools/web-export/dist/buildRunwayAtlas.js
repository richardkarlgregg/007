...
// Pixel helpers
// ---------------------------------------------------------------------------
function expand3to8(v) {
    return ((v << 5) | (v << 2) | (v >> 1)) & 0xff;
}
function expand4to8(v) {
    return ((v << 4) | v) & 0xff;
}
function expand5to8(v) {
    return ((v << 3) | (v >> 2)) & 0xff;
}
function decodeRgba5551ToRgba8888(value) {
    return [
...
function decodeIa88ToRgba8888(value) {
 ...
 */
function rawPixelToRgba(raw, format) {
    switch (format) {
        case 0x00: // RGBA32
...
// ---------------------------------------------------------------------------
function channelScratchToRgba8888(scratch, n, format) {
...
// ---------------------------------------------------------------------------
function texInflateHuffman(dst, dstOffset, numiterations, chansize, br) {
    if (chansize <= 0 || numiterations <= 0)
        return;
...
// ---------------------------------------------------------------------------
function texInflateRle(dst, dstOffset, blockstotal, br) {
...
// ---------------------------------------------------------------------------
function texReadAlphaBits(dst, dstOffset, count, br) {
...
// ---------------------------------------------------------------------------
function texBlur(pixels, width, height, method, chansize) {
...
// ---------------------------------------------------------------------------
function texGetBitSize(decimal) {
    let count = 0;
    decimal--;
    while (decimal > 0) {
        decimal >>>= 1;
...
// ---------------------------------------------------------------------------
function texBuildLookupPalette(format, bpp, br) {
...
// ---------------------------------------------------------------------------
function texLookupToRgba8888(br, width, height, palette, numcolors) {
...
// ---------------------------------------------------------------------------
function texLookupFromBufToRgba8888(indices, width, height, palette, numcolors) {
    const rgba = new Uint8Array(width * height * 4);
    for (let i = 0; i < width * height; i++) {
...
// ---------------------------------------------------------------------------
function texDecodeUncompressed(br, width, height, format) {
    const rgba = new Uint8Array(width * height * 4);
    for (let y = 0; y < height; y++) {
...
// ---------------------------------------------------------------------------
function tryInflateRzipChunk(chunk) {
    const attempts = [
...
// ---------------------------------------------------------------------------
function decodeZlibTexture(bytes) {
    if (bytes.length < 4)
        return null;
...
// HUFFMANBLUR, RLEBLUR).
// ---------------------------------------------------------------------------
function decodeNonZlibTexture(bytes) {
    ...
    if (width <= 0 || height <= 0 || width * height > 0x8000)
        return null;
    if (format > 12)
        return null;
    ...
    try {
        ...
        switch (compmethod) {
            ...
            case 2: { // HUFFMAN — single Huffman tree for all channel planes
                ...
                if (has1BitAlpha)
                    texReadAlphaBits(scratch, n * 3, n, br);
...
// parseImageNames
// ---------------------------------------------------------------------------
function parseImageNames(imagesDefPath) {
...
// blitRgba
// ---------------------------------------------------------------------------
function blitRgba(dst, dstWidth, src, srcWidth, srcHeight, dx, dy) {
...
export function buildRunwayAtlas(repoRoot, materialIds) {
    ...
    console.log(`Atlas decode: ${zlibCount} zlib, ${nonZlibCount} non-zlib, ${fallbackCount} fallback` +
