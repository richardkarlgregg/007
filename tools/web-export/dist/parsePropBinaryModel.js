...
function resolveModelRootOffset(binary, allowAlternateHeaderRoots) {
    ...
    if (allowAlternateHeaderRoots) {
        ...
        if (binary.length >= 0x18)
            candidateRoots.add(0);
        ...
        const isPlausibleRoot = (root) => {
            if (root + 0x18 > binary.length)
                return false;
        ...
        for (const root of candidateRoots) {
            if ((readU16BE(binary, root) & 0xff) !== 0x01)
                continue;
