 ...
 */
function parseJointListFromC(source, skelName) {
    const escapedName = skelName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const re = new RegExp(`ModelJoint\\s+JOINTLIST\\(${escapedName}\\)\\s*\\[\\]\\s*=\\s*\\{([\\s\\S]*?)\\};`);
 ...
 */
function findSkeletonName(headersSource, modelName) {
    const escaped = modelName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const re = new RegExp(`MODELFILEHEADER\\(${escaped}\\s*,.*?&SKELETON\\((\\w+)\\)`);
 ...
 */
export function loadSkeleton(repoRoot, modelName) {
 ...
 */
function parseU32ArrayFromC(source, arrayName, defines) {
    const escapedName = arrayName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const re = new RegExp(`u32\\s+${escapedName}\\s*\\[\\]\\s*=\\s*\\{([\\s\\S]*?)\\};`);
    const m = source.match(re);
    if (!m)
        return null;
    const body = m[1];
 ...
 */
export function parseAnimHeader(name, dataBuf) {
 ...
 */
function extractBits(buf, bitOffset, width) {
    let result = 0;
 ...
 */
function binaryAngleToRadians(raw, bitsPerComponent) {
 ...
 */
export function decodeAnimationFrames(header, entryBuf, skeleton) {
 ...
 */
export function loadAnimation(repoRoot, animName) {
    const dataSource = readFileSync(`${repoRoot}/assets/animationtable_data.c`, "utf8");
    // Parse PTR_ANIM_ENTRY_* defines so symbolic constants in ANIM_DATA arrays resolve
    const entryDefines = parseDefines(readFileSync(`${repoRoot}/assets/animationtable_entries.h`, "utf8"));
    ...
    const entrySource = readFileSync(`${repoRoot}/assets/animationtable_entries.c`, "utf8");
    ...
    if (entryBuf.length < expectedSize) {
        throw new Error(`ANIM_ENTRY_${animName} too short: ${entryBuf.length} bytes, ` +
